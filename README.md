...
## 首次部署（管理员）
1. **建表**：Supabase 控制台 → SQL Editor → 粘贴执行 `schema.sql`（sbp_ 令牌无建表权限，必须手动执行）
2. **部署函数**：
   ```powershell
   ...
   ```
3. **GitHub Secrets**（定时任务用）：`SUPABASE_URL`、`SUPABASE_ANON_KEY`（匿名公钥，前端 config.js 里也有）
